...
function calculateNetSalary(grossSalary) {
    // Input validation: Check if the input is a valid positive number
    if (isNaN(grossSalary) || grossSalary <= 0) {
        throw new Error("Invalid input: Gross salary must be a positive number.");
    }

    // Income Tax: 20% of the gross salary
    const incomeTax = grossSalary * 0.20;

    // Social Security Contributions based on the gross salary
    ...
    if (grossSalary <= 500000) {
        ...
        socialSecurity = 87500;  // Maximum social security contribution
    }

    // Stamp Duty based on the gross salary
    let stampDuty = 0;
...
// Example usage
try {
    const grossSalary = 1200000; // Example gross salary
